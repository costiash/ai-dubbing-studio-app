...
def convert_to_mp3(uploaded_file: Any) -> str:
...
if st.session_state.api_client:
    ...
    tts_voice = st.sidebar.selectbox(
        "Voice", ["alloy", "echo", "fable", "onyx", "nova", "shimmer"], index=3
    )
...
# Step 1: Transcribe
if uploaded_file and st.session_state.api_client and st.button("📝 Step 1: Transcribe Audio"):
...
if st.session_state.transcription_text:
    ...
    edited_text = st.text_area(
        "Correct any errors before translation:", st.session_state.transcription_text
    )
            ...
            try:
                ...
                trans_resp = st.session_state.api_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": f"Translate {input_lang} to {output_lang}. Be natural.",
                        },
                        {
                            "role": "user",
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp:
                    for chunk in speech_resp.iter_bytes():
                        tmp.write(chunk)
